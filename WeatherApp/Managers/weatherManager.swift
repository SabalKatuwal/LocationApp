...
let weatherAPI = Config.weatherApiKey
...
class WeatherManager{
    ...
    func getCurrentWeather(latitude: CLLocationDegrees, longitude: CLLocationDegrees)async throws -> ResponseBody {
        ...
        guard let url = URL(string: "https://api.openweathermap.org/data/2.5/weather?lat=\(latitude)&lon=\(longitude)&appid=\(weatherAPI)&units=metric") else { fatalError("Missing URL") }
